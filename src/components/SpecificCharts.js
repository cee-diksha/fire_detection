...
export const SpecificBattChart = ({batt, status}) => {
    ...
    const lineColor =  status.includes("yellow") ? "#f8ef51" :status.includes("deleted") ? "#b0acac" : status.includes("not responding") ? "#a391b8" : "#05abf9"
...
export const SpecificTempChart = ({temperature, status}) => {
    ...
    const lineColor = status.includes("danger") ? "#ff7b7b" : status.includes("orange") ? "#ff9863" : status.includes("not responding") ? "#a391b8" : "#05abf9"
    return(
        <LineChart
            xAxis={[{ data: time, scaleType: 'band', label: "Time(hrs)", valueFormatter: (value, context) => context.location === "tick" ? value : `${value}00 hours`}]}
            yAxis={[{
